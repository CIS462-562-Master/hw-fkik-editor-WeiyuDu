...
ATransform ATransform::Inverse() const
{
	...
	ATransform inverse;
	inverse.m_rotation = m_rotation.Inverse();
	inverse.m_translation = -inverse.m_rotation * m_translation;
	return inverse;
}
...
vec3 ATransform::RotTrans(const vec3& vecToTransform) const
{
	...
	vec3 res = m_rotation * vecToTransform + m_translation;
	return res;
}
...
vec3 ATransform::Rotate(const vec3& vecToTransform) const
{
	...
	vec3 res = m_rotation * vecToTransform;
	return res;
}
...
vec3 ATransform::Translate(const vec3& vecToTransform) const
{
	...
	vec3 res = vecToTransform + m_translation;
	return res;
}
...
ATransform operator * (const ATransform& H1, const ATransform& H2)
{
	...
	res.m_rotation = H1.m_rotation * H2.m_rotation;
	res.m_translation = H1.m_rotation * H2.m_translation + H1.m_translation;
